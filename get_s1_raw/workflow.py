...
class GetAcquriredProductsFromLastRun(luigi.ExternalTask):
    ...
    def output(self):
        lastDate = self.runDate - timedelta(days=1)
        return wc.getTarget('S1RawProducts.json', lastDate, getStatePaths(self.pathroots), self.useLocalTarget)
...
class GetRawDataFeed(luigi.Task):
    ...
    def getBaseQuery(self):
        ...
        # defaults to the rundateT00:00:00
    ...
    def run(self):
        ...
        if totalResults > 10000:
            log.exception(
                "Total number of results exceeds 10,000 and cannot be retreived")
...
class GetS1RawProducts(luigi.Task):
    useLocalTarget = luigi.BoolParameter()
    ...
    def output(self):
        return wc.getTarget('S1RawProducts.json', self.runDate, getStatePaths(self.pathroots), self.useLocalTarget)  
